...
class DefaultKVideoEmbedBuilder extends quill.EmbedBuilder {
  final Widget Function(KCustomVideoEmbedData embedData, quill.Embed node, bool readOnly,
  ...
  Widget build(BuildContext context, quill.QuillController controller, quill.Embed node,
    ...
    try {
      if (node.value.type == key) {
        final KCustomVideoEmbedData data =
            KCustomVideoEmbedData.fromMap(jsonDecode(node.value.data as String));
              ...
              builder: (BuildContext context, Size size) {
                return AspectRatio(
                  ...
                  child: Container(
                      ...
                      child: const Center(child: Text("Video Embed Implementation"))),
                );
              },
            );
      }
